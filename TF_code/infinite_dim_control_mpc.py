...
with tf.device('/CPU:0'):
	...
	def rolls_split(Xi_roll): # each input will be of shape (T, J-1, J-1)
		...
		for t_ in range(T):
			...
			temp_list.append(tf.reshape(tf.transpose(dW_mat, perm=[1,0]), [1,-1])) # Transpose is required before flattening reshape for vec operation
...
with tf.device(ops_device):
	...
	if terminal_only:
		...
		print("")
		J_h = scale_factor * tf.expand_dims( tf.reduce_sum(tf.square(h_ns_sliced[:,-1,:,:] - tf.expand_dims(h_d_sliced,0)), axis=[1,2], keepdims=False), -1) # final shape : (rollouts,1)
	else:
...
with tf.Session() as sess:
	...
	all_cost = np.zeros((Tsim_steps,1))
	...
	for t_sim in range(Tsim_steps):
		...
		h_vec = np.reshape(h_current.T, [-1,1]) # vec operation
		v_n = np.dot(np.expand_dims(U_current[0,:],0), curly_v_tilde).T # operation : (1 x N) x (N x (J-1)*(J-1)) ---(after transpose)---> ((J-1)*(J-1) x 1)
		...
		h_current = np.reshape(h_new, (J-1,J-1)).T # Convert from vec to 2D mat for next timestep
		U_current = np.concatenate((U_current[1:,:], np.expand_dims(U_current[-1,:],0)), axis=0) # Warm-start

		cost = 0
		for i in range(len(r_x1)):
			cost = cost + scale_factor * np.sum(np.square(h_d[ r_y1[i]:r_y2[i]+1, r_x1[i]:r_x2[i]+1 ] - h_current[ r_y1[i]:r_y2[i]+1, r_x1[i]:r_x2[i]+1 ]))

		endtime = time()
		sys.stdout.write("step:{} of {}, cost: {:5f}, iter time: {:.5f}\r".format(t_sim+1, Tsim_steps, cost, endtime - starttime))
		sys.stdout.flush()
